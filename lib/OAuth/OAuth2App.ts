...
export class OAuth2App extends Homey.App {
  ...
  async onInit() {
    ...
    if (
      this.constructor.OAUTH2_CLIENT.API_URL &&
  ...
  setOAuth2Config({
    ...
    if (
      !client ||
      (client !== OAuth2Client &&
        client.prototype instanceof OAuth2Client !== true)
    ) {
    ...
    if (
      typeof authorizationUrl !== 'undefined' &&
  ...
  saveOAuth2Client({ configId, sessionId, client }) {
    ...
    savedSessions[sessionId] = {
      configId,
      title,
      token: token instanceof OAuth2Token ? token.toJSON() : null,
    };
    this.homey.settings.set(SETTINGS_KEY, savedSessions);
  }
